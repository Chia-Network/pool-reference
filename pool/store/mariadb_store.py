...
class MariadbPoolStore(AbstractPoolStore):
    ...
    async def connect(self) -> None:
        ...
        self.connection = await self.pool_db.acquire()
        ...
        self.pool_db.release(self.connection)

    @staticmethod
    def _row_to_farmer_record(row) -> FarmerRecord:
        return FarmerRecord(
            bytes32(bytes.fromhex(row[0])),
            bytes32(bytes.fromhex(row[1])),
            row[2],
            bytes32(bytes.fromhex(row[3])),
    ...
    async def add_farmer_record(self, farmer_record: FarmerRecord, metadata: RequestMetadata):
        with await self.pool_db as connection:
            cursor = await connection.cursor()
            await cursor.execute(
                "INSERT INTO farmer VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s) "
                "ON DUPLICATE KEY UPDATE p2_singleton_puzzle_hash=%s, delay_time=%s, delay_puzzle_hash=%s,"
                "authentication_public_key=%s, singleton_tip=%s, singleton_tip_state=%s, payout_instructions=%s, "
                "is_pool_member=%s",
                (
    ...
    async def get_farmer_record(self, launcher_id: bytes32) -> Optional[FarmerRecord]:
        ...
        with await self.pool_db as connection:
            cursor = await connection.cursor()
            await cursor.execute(
                "SELECT * FROM farmer WHERE launcher_id=%s",
                (launcher_id.hex(),),
            )
    ...
    async def update_difficulty(self, launcher_id: bytes32, difficulty: uint64):
        with await self.pool_db as connection:
            connection = await self.pool_db.acquire()
            cursor = await connection.cursor()
            await cursor.execute(
    ...
    async def get_farmer_points_and_payout_instructions(self) -> List[Tuple[uint64, bytes]]:
        with await self.pool_db as connection:
            cursor = await connection.cursor()
            await cursor.execute("SELECT points, payout_instructions FROM farmer")
            rows = await cursor.fetchall()
            await cursor.close()
            ...
            for row in rows:
                ...
                if ph in accumulated:
                    accumulated[ph] = uint64(accumulated[ph] + points)
                else:
                    accumulated[ph] = points

            ret: List[Tuple[uint64, bytes]] = []
    ...
    async def clear_farmer_points(self) -> None:
        with await self.pool_db as connection:
            cursor = await connection.cursor()
            await cursor.execute("UPDATE farmer SET points=0")
            await cursor.close()
            await connection.commit()
